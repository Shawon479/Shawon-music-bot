...
async def _controls(_, query: types.CallbackQuery):
    ...
    if getattr(config, "MAINTENANCE", False) and query.from_user.id != config.OWNER_ID:
        return await query.answer("Bot is currently under maintenance. Please try again later.", show_alert=True)
    ...
    if action == "pause":
        ...
        if start_from + 5 > media.duration_sec:
            return await query.answer("Already at the end.", show_alert=True)
        media.time = start_from
        await anon.play_media(chat_id, query.message, media, start_from)
        return await query.answer(f"Seeked forward to {start_from}s")
        ...
        media.time = start_from
        await anon.play_media(chat_id, query.message, media, start_from)
        return await query.answer(f"Seeked back to {start_from}s")
